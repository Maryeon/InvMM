...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        "-c", "--config", type=str, default="configs/stable-diffusion/inversion.yaml",
        help="config file"
    )
    parser.add_argument(
        "--ckpt_file", default="models/stable-diffusion/v2-1_512-ema-pruned.ckpt", type=str,
...
def load_model_from_config(config, ckpt, device=torch.device("cuda:0"), verbose=False):
...
def main():
    ...
    model = load_model_from_config(config, args.ckpt_file, device=device)

    sampler = DDIMSampler(model, deivce=device)
...
class Trainer(object):
    ...
    def sample_tokens(self, log_coeffs, tau=1., hard=True):
        tokenizer = open_clip.tokenizer._tokenizer
        coeffs = F.gumbel_softmax(log_coeffs, hard=hard, tau=tau)
        token_ids = coeffs.argmax(dim=1)
        prompt = tokenizer.decode(token_ids.cpu().numpy())
    ...
    def train_one_batch(
            self, save_dir, z, log_coeffs, img, pseudo_prompt, img_id,
        ...
        for step in tqdm(range(self.train_num_steps), dynamic_ncols=True):
            ...
            if self.vis is not None:
                self.add_scalar(step, p_losses.global_avg, f"{img_id}_p_loss")
                self.add_scalar(step, r_losses.global_avg, f"{img_id}_r_loss")
    ...
    def train(self):
        for batch_idx, (img, id) in enumerate(self.dl):
            ...
            self.train_one_batch(work_dir, z, log_coeffs, img, pseudo_prompt, id[0])
...
class LatentDiffusionInversion(LatentDiffusion):
    def __init__(self, *args, **kwargs):
        ...
        voc_emb = self.cond_stage_model.model.token_embedding.weight.detach().clone()
    ...
    def forward(self, z, n, txts, tok_emb, num_tokens=3, num_samples=8):
        ...
        p_loss = self.p_losses(z, c, t, noise=n)
    ...
    def modify_token_embedding(self, tok_emb, num_tokens):
        def hook(module, args, output):
            output[:, 1:1+num_tokens] = tok_emb
            return output
        
        embedder = self.cond_stage_model.model.token_embedding
