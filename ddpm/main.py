...
flags.DEFINE_bool('dup', False, help='duplicate training set')
# UNet
...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if FLAGS.dup:
            self.paths = self.paths[:10] * 100 + self.paths[10:]
...
def train():
    ...
    #         transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
    #     ]))
    dataset = Dataset(folder='/data/home/mz/datasets/cifar10/imgs')
    ...
    x_T = x_T.to(device)
    grid = (make_grid(next(iter(dataloader))[:FLAGS.sample_size]) + 1) / 2
